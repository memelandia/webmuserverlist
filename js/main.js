...
async function loadFeaturedCarousel() {
    ...
    try {
        const { data, error } = await window.supabaseClient.from('servers').select('id, name, image_url, banner_url, version, type, configuration, exp_rate, drop_rate, opening_date').eq('status', 'aprobado').eq('is_featured', true).limit(5);
        if (error) throw error;
        if (!data || data.length === 0) { container.innerHTML = '<p style="padding: 2rem; text-align: center;">No hay servidores destacados.</p>'; return; }
        container.innerHTML = data.map(server => {
            ...
            const optimizedLogo = getOptimizedImageUrl('server-images', server.image_url, { width: 180, height: 180 }, 'https://via.placeholder.com/90');
            ...
            <div class="carousel-card">
                ...
                <div class="carousel-card-info">
                    <img src="${optimizedLogo}" alt="Logo de ${server.name}" class="carousel-card-logo">
                    <div>
                        ...
                        <div class="carousel-card-rates">
                            ...
                            <span><i class="fa-solid fa-gem"></i> ${server.drop_rate || '?'}%</span>
                        </div>
        ...
        initCarousel();
    } catch (error) { container.innerHTML = `<p class="error-text">No se pudieron cargar los servidores.</p>`; }
}
...
function initCarousel() {
    ...
    const updateButtons = () => { const canScroll = carousel.scrollWidth > carousel.clientWidth; prevBtn.style.display = canScroll ? 'block' : 'none'; nextBtn.style.display = canScroll ? 'block' : 'none'; };
    setTimeout(() => {
        updateButtons();
...
async function loadStats() {
    ...
    try {
        const { count: serverCount } = await window.supabaseClient.from('servers').select('*', { count: 'exact', head: true }).eq('status', 'aprobado');
        const { count: userCount } = await window.supabaseClient.from('profiles').select('*', { count: 'exact', head: true });
        const { data: servers, error } = await window.supabaseClient.from('servers').select('votes_count').eq('status', 'aprobado');
