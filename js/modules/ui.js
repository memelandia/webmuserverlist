// js/modules/ui.js (v18 - Completo con todas las funciones de renderizado)
...
export function renderError(container, message = "Error al cargar.") {
    if(container) container.innerHTML = `<p class="error-text">${message}</p>`;
}
...
export function renderUserProfile(container, data) { const { session, profile, servers, reviews } = data, t = getOptimizedImageUrl("avatars", profile.avatar_url, { width: 300, height: 300, resize: "cover" }, "img/avatar_default.png"), n = servers && servers.length > 0 ? servers.map(server => ` <div class="detail-card"> <img src="${getOptimizedImageUrl('server-images',server.image_url,{width:90,height:90},'img/logo_placeholder_small.png')}" class="server-logo-table" loading="lazy" alt="Logo de ${server.name}"> <h4><a href="servidor.html?id=${server.id}" class="server-name-link">${server.name}</a></h4> <span class="status-tag status-${server.status||"pendiente"}">${server.status||"pendiente"}</span> <div class="actions"> <a href="servidor.html?id=${server.id}" class="btn btn-sm btn-secondary">Ver</a> ${profile.id===server.user_id||"admin"===profile.role?`<a href="editar-servidor.html?id=${server.id}" class="btn btn-sm btn-primary">Editar</a>`:""} </div> </div>`).join("") : '<p>Aún no has añadido ningún servidor. <a href="agregar.html">¿Tienes uno?</a></p>', r = reviews && reviews.length > 0 ? reviews.map(review => ` <div class="user-review-card"> <div class="review-header"> <div class="review-user-info"> <strong>Tu reseña para <a href="servidor.html?id=${review.servers.id}">${review.servers?.name||"Servidor Eliminado"}</a></strong> <span class="review-date">${new Date(review.created_at).toLocaleDateString("es-ES")}</span> </div> <div class="review-stars">${renderStars(review.rating)}</div> </div> ${review.comment?`<p class="review-comment">“${review.comment}”</p>`:""} </div>`).join("") : "<p>Aún no has dejado ninguna reseña.</p>"; container.innerHTML = ` <div class="page-header"><h1><i class="fa-solid fa-user-circle"></i> Mi Perfil</h1></div> <div class="profile-grid"> <aside class="profile-sidebar"> <div class="widget" style="text-align:center;"> <div class="profile-avatar-container"> <img src="${t}" alt="Avatar de ${profile.username}" id="profile-avatar-img" class="profile-avatar-img"> <label for="avatar-upload-input" id="avatar-upload-label" title="Cambiar avatar"><i class="fa-solid fa-camera"></i></label> <input type="file" id="avatar-upload-input" accept="image/png, image/jpeg, image/gif"> </div> <div id="avatar-feedback"></div> <h2>${profile.username||"Usuario"}</h2> <p class="text-secondary" style="word-wrap:break-word;">${session.user.email}</p> <p class="text-secondary">Rol: <span style="text-transform: capitalize;">${profile.role}</span></p> <small class="text-secondary">Miembro desde: ${new Date(session.user.created_at).toLocaleDateString("es-ES")}</small> </div> </aside> <main class="profile-main-content"> <div class="widget"> <h3 class="widget-title">Mis Servidores Publicados</h3> <div class="servers-list">${n}</div> </div> <div class="widget"> <h3 class="widget-title">Mis Reseñas Recientes</h3> <div class="user-reviews-list">${r}</div> </div> </main> </div>` }
export function renderOwnerDashboard(container,data){const{session:e,profile:t,servers:n,dashboardStats:r}=data,i=getOptimizedImageUrl("avatars",t.avatar_url,{width:300,height:300,resize:"cover"},"img/avatar_default.png"),o=(r||[]).reduce((e,t)=>(e.views+=t.view_count||0,e.webClicks+=t.website_click_count||0,e.discordClicks+=t.discord_click_count||0,e.votes+=t.votes_count||0,e),{views:0,webClicks:0,discordClicks:0,votes:0}),a=n&&n.length>0?n.map(e=>` <div class="detail-card"> <img src="${getOptimizedImageUrl("server-images",e.image_url,{width:90,height:90},"img/logo_placeholder_small.png")}" class="server-logo-table" loading="lazy" alt="Logo de ${e.name}"> <h4><a href="servidor.html?id=${e.id}" class="server-name-link">${e.name}</a></h4> <span class="status-tag status-${e.status||"pendiente"}">${e.status||"pendiente"}</span> <div class="actions"> <a href="servidor.html?id=${e.id}" class="btn btn-sm btn-secondary">Ver</a> <a href="editar-servidor.html?id=${e.id}" class="btn btn-sm btn-primary">Editar</a> </div> </div>`).join(""):`<p>Aún no has añadido ningún servidor. <a href="agregar.html">¡Publica el primero!</a></p>`;container.innerHTML=` <div class="page-header"><h1><i class="fa-solid fa-chart-line"></i> Dashboard de Servidores</h1></div> <div class="profile-grid"> <aside class="profile-sidebar"> <div class="widget" style="text-align:center;"> <div class="profile-avatar-container"> <img src="${i}" alt="Avatar de ${t.username}" id="profile-avatar-img" class="profile-avatar-img"> <label for="avatar-upload-input" id="avatar-upload-label" title="Cambiar avatar"><i class="fa-solid fa-camera"></i></label> <input type="file" id="avatar-upload-input" accept="image/png, image/jpeg, image/gif"> </div> <div id="avatar-feedback"></div> <h2>${t.username||"Usuario"}</h2> <p class="text-secondary" style="word-wrap:break-word;">${e.user.email}</p> <p class="text-secondary">Rol: <span style="text-transform: capitalize;">${t.role}</span></p> </div> </aside> <main class="profile-main-content"> ${n&&n.length>0?` <div class="widget"> <h3 class="widget-title">Rendimiento Global</h3> <div class="owner-stats-grid"> <div class="stat-card"><div class="stat-card-icon"><i class="fa-solid fa-eye"></i></div><div class="stat-card-value">${o.views.toLocaleString("es-ES")}</div><div class="stat-card-label">Vistas Totales</div></div> <div class="stat-card"><div class="stat-card-icon"><i class="fa-solid fa-mouse-pointer"></i></div><div class="stat-card-value">${o.webClicks.toLocaleString("es-ES")}</div><div class="stat-card-label">Clics a Web</div></div> <div class="stat-card"><div class="stat-card-icon"><i class="fab fa-discord"></i></div><div class="stat-card-value">${o.discordClicks.toLocaleString("es-ES")}</div><div class="stat-card-label">Clics a Discord</div></div> <div class="stat-card"><div class="stat-card-icon"><i class="fa-solid fa-heart"></i></div><div class="stat-card-value">${o.votes.toLocaleString("es-ES")}</div><div class="stat-card-label">Votos Totales</div></div> </div> <div class="owner-charts-grid"> <div class="widget" style="margin-bottom:0;"><h3 class="widget-title" style="margin-bottom: 1.5rem">Interacciones</h3><canvas id="interactions-chart"></canvas></div> <div class="widget" style="margin-bottom:0;"><h3 class="widget-title" style="margin-bottom: 1.5rem">Distribución de Votos</h3><canvas id="votes-chart"></canvas></div> </div> </div> `:""} <div class="widget"> <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem;"><h3 class="widget-title" style="margin:0;border:0;">Mis Servidores</h3><a href="agregar.html" class="btn btn-primary"><i class="fa-solid fa-plus"></i> Añadir Nuevo</a></div> <div class="servers-list">${a}</div> </div> </main> </div>`}
export function initOwnerCharts(stats) { const e = document.getElementById("interactions-chart")?.getContext("2d"), t = document.getElementById("votes-chart")?.getContext("2d"); if (!e || !t) return; const n = stats.map(e => e.name.substring(0, 20)); new Chart(e, { type: "bar", data: { labels: n, datasets: [{ label: "Vistas", data: stats.map(e => e.view_count), backgroundColor: "rgba(255, 51, 51, 0.5)" }, { label: "Clics Web", data: stats.map(e => e.website_click_count), backgroundColor: "rgba(51, 153, 255, 0.5)" }, { label: "Clics Discord", data: stats.map(e => e.discord_click_count), backgroundColor: "rgba(114, 137, 218, 0.5)" }] }, options: { responsive: !0, maintainAspectRatio: !1, scales: { y: { beginAtZero: !0, ticks: { color: "var(--text-secondary)" } }, x: { ticks: { color: "var(--text-secondary)" } } }, plugins: { legend: { labels: { color: "var(--text-primary)" } } } } }), new Chart(t, { type: "doughnut", data: { labels: n, datasets: [{ label: "Votos", data: stats.map(e => e.votes_count), backgroundColor: ["#ff3333", "#b42424", "#8b1d1d", "#e62e2e", "#621616", "#410e0e"], borderColor: "var(--bg-light)", borderWidth: 2 }] }, options: { responsive: !0, maintainAspectRatio: !1, plugins: { legend: { position: "top", labels: { color: "var(--text-primary)" } } } } }) }

// --- Componentes del Panel de Administración (sin cambios) ---
export function renderAdminPendingServers(container,servers){if(!servers||0===servers.length)return void(container.innerHTML="<p>No hay servidores pendientes de aprobación.</p>");container.innerHTML=servers.map(e=>` <div class="detail-card"> <h4>${e.name} <small>(${e.version})</small></h4> <div class="actions"> <button data-id="${e.id}" class="btn btn-sm btn-primary approve-btn">Aprobar</button> <a href="servidor.html?id=${e.id}" target="_blank" class="btn btn-sm btn-secondary">Ver</a> <button data-id="${e.id}" class="btn btn-sm btn-danger deny-btn">Rechazar</button> </div> </div>`).join("")}
export function renderAdminAllServers(container,servers){container.innerHTML=servers.map(e=>` <div class="detail-card"> <h4>${e.name} <span class="status-tag status-${e.status}">${e.status}</span></h4> <div class="actions"> <span>Destacado:</span> <label class="switch"> <input type="checkbox" class="featured-toggle" data-id="${e.id}" ${e.is_featured?"checked":""}> <span class="slider"></span> </label> <a href="editar-servidor.html?id=${e.id}" class="btn btn-sm btn-secondary">Editar</a> <button data-id="${e.id}" class="btn btn-sm btn-danger deny-btn">Eliminar</button> </div> </div>`).join("")}
export function renderAdminUsers(container,users){container.innerHTML=users.map(e=>` <div class="detail-card"> <h4>${e.username||e.email}</h4> <div class="actions"> <span>Rol:</span> <select class="user-role-select" data-id="${e.id}"> <option value="player" ${"player"===e.role?"selected":""}>Player</option> <option value="owner" ${"owner"===e.role?"selected":""}>Owner</option> <option value="admin" ${"admin"===e.role?"selected":""}>Admin</option> </select> </div> </div>`).join("")}
export function renderAdminSoM(container,{currentWinner:e,allServers:t}){let n=` <div id="som-current-winner"> <i class="fa-solid fa-trophy fa-3x" style="color: var(--text-secondary);"></i> <div> <h4>Aún no hay ganador</h4> <p>Selecciona un servidor de la lista.</p> </div> </div>`;if(e){const t=getOptimizedImageUrl("server-images",e.image_url,{width:160,height:160},"img/logo_placeholder.png");n=` <div id="som-current-winner"> <img src="${t}" alt="Logo de ${e.name}"> <div> <h4>Ganador Actual: ${e.name}</h4> <p>ID: ${e.id}</p> </div> </div>`}const r=t.map(e=>`<option value="${e.id}">${e.name} (ID: ${e.id})</option>`).join("");container.innerHTML=` ${n} <form id="som-selection-form" style="margin-top:2rem;"> <div class="form-group"> <label for="som-select" style="font-size:1.2rem; font-weight:600;">Seleccionar Nuevo Ganador</label> <select id="som-select" required style="padding: 0.8rem 1rem; width:100%; background:#252525; border:1px solid #333; color:white; border-radius:8px;"> <option value="">-- Elige un servidor --</option> ${r} </select> </div> <button type="submit" class="btn btn-primary btn-lg" style="margin-top:1rem;">Establecer como Ganador</button> </form> <div id="som-feedback" class="feedback-message" style="margin-top: 1rem;"></div>`}